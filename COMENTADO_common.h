 ...
 */
.macro BEGIN
    LOCAL after_locals  /* LOCAL define las variables locales a la macro */
    .code16             /* código de 16 bits porque el cpu arranca en modo real */
    cli                 /* Se desactivan las interrupciones antes de configurar los registros de segmento
                           ya que de producirse trataría de usar código del manejador de interrupciones, 
                           cargando por lo menos el registro de segmento CS con lo que encuentre en la IDT*/
    /* Set %cs to 0. TODO Is that really needed? */
    /* */

    /* Salto lejano a una etiqueta local.
    Objetivo: setear en CS un segmento de código conocido
    NO ES NECESARIO QUE SEA CERO*/
    ljmp $0, $1f 
    1:
    /* Inicializar los registros de datos a cero. 
       DS debe ser cero para cualquier acceso a datos.
       ES no necesita configurarse a menos que una rutina de BIOS en particular 
       lo requiera como parte de un parámetro que se le está pasando. 
       Las rutinas de BIOS no requieren que se configuren FS y GS. */
    xor %ax, %ax    /* ax = 0*/
    mov %ax, %ds   
    mov %ax, %es
    mov %ax, %fs
    ...
    /* No es necesario inicializar BP. Si una llamada a la BIOS requiere BP como parámetro, 
       la documentación listará los registros requeridos y solo debe configurarse antes de la llamada.
       La cima de la pila (SS:SP) se coloca en la base del segmento de 64 KB. 
       Las interrupciones deben estar deshabilitadas */
    mov %ax, %bp /* BP = SS = SP = 0*/
    mov %ax, %ss
    mov %bp, %sp 
    ...
    /* Antes de saltar a 0x7c00, el BIOS carga en DL el "drive number", que indica el dispositivo del cual va a bootear. 
       0x80 indica que arranque (bootee) del disco duro
       0x00 indica que arranque (bootee) de la unidad de disquete, ¿cd-rom, pendrive, etc.?
    */
    mov %dl, initial_dl     /* DL = Data Low (8 bits inferiores) */
    jmp after_locals
    initial_dl: .byte 0     /* Lo utiliza en la macro STAGE2, para inicializar DL antes de saltar a la int 13 de la BIOS */
after_locals:
.endm
...
.macro STAGE2
    ...
    mov $__stage2_nsectors, %al /* AL = Cantidad de sectores a leer */
    mov $0x02, %ah          /* AH = Identifica Operación: leer sectores desde el dispositivo */
    mov $1f, %bx            /* ES(0x0000):BX(et 1f) = Dirección del buffer de datos */
    mov $0x0002, %cx        /* CH(0x00) = N° Cilindro, CL(0x02) = N° Sector */
    mov $0x00, %dh          /* DH = N° Cabeza */
    mov initial_dl, %dl     /* DL = N° Unidad = 0x00 (BEGIN) = 1° Disquetera (Unidad A) */
    int $0x13               /* BIOS int 13 = Operación típicas de acceso a dispositivos */
    jmp 1f                  /* */
    .section .stage2
    1:
.endm
